...
module Terminal = struct
  include Indexed(Raw.Terminal)
  let all = all n
end

module Nonterminal = struct
  include Indexed(Raw.Nonterminal)
  let all = all n
end
...
module Symbol = struct
  ...
  let of_g = function
    | Raw.T t -> T (Terminal.of_g t)
    | Raw.N n -> N (Nonterminal.of_g n)

  let to_g = function
    | T t -> Raw.T (Terminal.to_g t)
    | N n -> Raw.N (Nonterminal.to_g n)

  let name ?mangled = function
    | T t -> Raw.symbol_name ?mangled (T (Terminal.to_g t))
    | N n -> Raw.symbol_name ?mangled (N (Nonterminal.to_g n))
end

module Production = struct
  include Indexed(Raw.Production)

  let lhs p = Nonterminal.of_g (Raw.Production.lhs (to_g p))

  let rhs =
    let import prod =
      Array.map
        (fun (sym,_,_) -> Symbol.of_g sym)
        (Raw.Production.rhs (to_g prod))
    in
    Vector.get (Vector.init n import)

  let kind p = Raw.Production.kind (to_g p)
end

module Lr1 = struct
  include Indexed(Raw.Lr1)
  let all = all n

  let to_lr0 lr1 = Raw.Lr1.lr0 (to_g lr1)

  let incoming lr1 =
    Option.map Symbol.of_g (Raw.Lr0.incoming (to_lr0 lr1))

  let items lr1 =
    List.map
      (fun (p,pos) -> (Production.of_g p, pos))
      (Raw.Lr0.items (to_lr0 lr1))
  ...
  let reductions =
    ...
    in
    let import_lr1 lr1 = import_red (Raw.Lr1.reductions (to_g lr1)) in
...
struct
  ...
  let shift_count, goto_count =
    ...
    (* Count goto and shift transitions by iterating on all states and
       transitions *)
    Raw.Lr1.iter begin fun lr1 ->
      List.iter begin fun (sym, _) ->
        match sym with
        | Raw.T _ -> incr shift_count
        | Raw.N _ -> incr goto_count
      end (Raw.Lr1.transitions lr1)
    end;
  ...
  let successors =
    ...
    Vector.init Lr1.n begin fun source ->
      List.fold_left begin fun acc (sym, target) ->
        ...
        index :: acc
      end [] (Raw.Lr1.transitions (Lr1.to_g source))
    end
