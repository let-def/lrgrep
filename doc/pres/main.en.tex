...
\begin{frame}{Characterizing errors (2/3): the error front}
  ...
  \mathcal E_L = \left\{ (u, t) \in \Sigma^*\times\Sigma\ \big|\
    \vext{viable}(u) \wedge \neg \vext{viable}(u.t)
  \right\} \\
  \qquad \text{where} \text{viable}(u) = \exists v \in \Sigma^*,\ u.v \in L
  $$
  \pause
...
\begin{frame}{Intuition on ``Shift-Reduce'' parsers}
  ...
  \begin{itemize}
    \item the input is consumed left to right \only<2->{({\em push})}
    \item the derivation is built starting from the leaves,
          that are progressively rooted \only<2->{({\em pop} leaves, {\em push} node)}
