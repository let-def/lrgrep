...
(*%on_error_reduce expr seq_expr simple_expr structure_item pattern simple_pattern
  core_type atomic_type toplevel_directive attr_id
  reversed_separated_nontrivial_llist(STAR,atomic_type) tuple_type alias_type
  reversed_separated_nonempty_llist(AMPERSAND,core_type_no_attr) row_field
  option(preceded(COLON,core_type)) option(preceded(EQUAL,pattern))
  simple_pattern_not_ident constr_longident pattern_comma_list(pattern)
  pattern_gen simple_pattern_not_ident separated_or_terminated_nonempty_list(SEMI,pattern)
  label_let_pattern functor_args module_type with_constraint module_expr let_pattern
  labeled_simple_expr  class_expr reversed_nonempty_llist(labeled_simple_expr)
  reversed_separated_nontrivial_llist(COMMA,expr) type_constraint
  pattern_comma_list(pattern_no_exn) pattern_no_exn
...
reversed_separated_nonempty_llist(COMMA,core_type)
class_signature
constrain_field
row_field
reversed_separated_nonempty_llist(COMMA,core_type)
...
mk_longident(mod_ext_longident,__anonymous_41)
...
reversed_nonempty_llist(name_tag)
reversed_nonempty_llist(typevar)
reversed_separated_nonempty_llist(AMPERSAND,core_type_no_attr)
reversed_separated_nonempty_llist(BAR,row_field)
reversed_separated_nonempty_llist(COMMA,core_type)
reversed_separated_nonempty_llist(COMMA,type_parameter)
reversed_separated_nonempty_llist(STAR,atomic_type)
