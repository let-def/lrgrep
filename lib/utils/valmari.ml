...
let index_rev_iter (n : _ cardinal) f =
...
let index_transitions (type state) (type transition)
  ...
  let a = Array.make (cardinal transitions)
      (Index.of_int transitions 0)
  in
  index_rev_iter transitions (fun t ->
    let state = (target t :> int) in
...
sig
  ...
  module States =
    Const(struct let cardinal = Partition.set_count blocks end)
  type states = States.n
  let states = States.n

  module Transitions = Const(struct
      let cardinal =
        let count = ref 0 in
  ...
  let transition_table =
    Vector.make' transitions (fun () -> Index.of_int In.transitions 0)

  let () =
    let next = Index.enumerate transitions in
    Index.iter In.transitions (fun tr ->
        if Partition.is_first blocks (In.source tr) &&
           Partition.set_of blocks (In.target tr) > -1
        then Vector.set transition_table (next()) tr
      )

  type label = Label.t

  let transport_state_unsafe =
    let table = Vector.init In.states (Partition.set_of blocks) in
    Vector.get table
  ...
  let represent_transition =
    Vector.get transition_table
  ...
  let transport_transition =
    ...
    Index.iter transitions (fun tr ->
        let trin = Vector.get transition_table tr in
