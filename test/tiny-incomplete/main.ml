...
(* This variant deliberately ships an INCOMPLETE error specification: the
 *   [OPAREN ; [chars]] rule of [test/tiny] has been removed. The whole point of
 *   this directory is that the build should FAIL at the lrgrep compilation step
 *   (which is run with [--cover-all]), before the driver below ever gets to run.
 *)
  ...
  let cases =
    [ ("()", "=> OK");
      ...
      (* The closing ')' is missing, and the error is reported at end of input
         ('$'). In the complete test this message comes from the rule
         [OPAREN ; [chars]]; here that rule is intentionally missing, so the
         build should fail before reaching this driver. *)
